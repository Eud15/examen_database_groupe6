...
```
for i in $(seq 1 6); do
  curl -s -D- -o /dev/null http://localhost:8080/v1/sante | grep -i x-instance
done
```
...
```
docker compose exec redis redis-cli -a momo2026 \
...
docker compose exec redis redis-cli -a momo2026 MEMORY USAGE bf:blacklist
```
...
```
docker compose exec redis redis-cli -a momo2026 SLOWLOG GET 5
```
...
docker compose exec web1 python manage.py simulate --scenario velocite
docker compose exec redis redis-cli -a momo2026 XPENDING stream:tx enrichissement
